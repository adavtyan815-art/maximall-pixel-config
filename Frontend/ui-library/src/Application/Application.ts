...
switch (response) {
     ...
     this.stream.config.setSettings({ HoveringMouse: true });
            console.log("Включаем HoveringMouse");
                ...
                break;
     case "DisableHoveringMouse":
            console.log("Выключаем HoveringMouse");
     this.stream.config.setSettings({ HoveringMouse: false });

break;
     case "SetBirate":
            console.log("Birates");
     this.stream.config.setSettings({ WebRTCMinBitrate: 3000 });
     this.stream.config.setSettings({ WebRTCMaxBitrate: 15000 });
     this.stream.config.setSettings({ MatchViewportRes: true });
     this.stream.config.setSettings({PreferredCodec:"H264+level-asymmetry-allowed%3D1%3Bpacketization-mode%3D1%3Bprofile-level-id%3D64001f"});
     break;
